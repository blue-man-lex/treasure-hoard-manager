...
export class GenericSystemAdapter extends SystemAdapter {
  ...
  getActorCurrency(actor) {
    const path = this.getCurrencyPath(actor);
  ...
  /**
   * Получение пути к валюте
   */
  getCurrencyPath(actor) {
    if (actor?.system?.wealth?.value !== undefined) return "system.wealth.value";
    if (actor?.system?.money !== undefined) return "system.money";
   ...
   */
  getItemRarity(item) {
    return this.getItemRarityData?.(item)?.value || 'common';
  }
  ...
  /**
   * Универсальная генерация цены
   */
  async generateItemPrice(item, method = 'default') {
    const config = this.getPricingConfig();
    const rarity = this.getItemRarity(item);
    const table = config.tables.default;
    const range = table[rarity] || table.common;
    ...
    return typeof range === 'number' ? range : 50;
  }
}
